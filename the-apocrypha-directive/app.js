(() => {
  ...
  const storage = {
    pageIndex: "bm.v32.pageIndex",
    muted: "bm.v32.muted",
    solved: "bm.v32.solved",
    inventory: "bm.v32.inventory",
    choices: "bm.v32.choices",
    redactions: "bm.v32.redactions",
    audioPlayed: "bm.v32.audioPlayed",
    unlockedMax: "bm.v32.unlockedMax"
  };
  ...
  function resetProgress() {
    const sure = confirm("Reset Black Meridian V3.2 local progress on this browser?");
    if (!sure) return;
    [
      ...Object.values(storage),
      "bm.pageIndex", "bm.muted", "bm.solved", "bm.inventory", "bm.choices", "bm.redactions", "bm.v31.pageIndex", "bm.v31.muted", "bm.v31.solved", "bm.v31.inventory", "bm.v31.choices", "bm.v31.redactions", "bm.v31.audioPlayed", "bm.v31.unlockedMax"
...
/* BLACK MERIDIAN // SIGNAL ANIMATION LAYER */
(function initBlackMeridianSignalFX(){if(window.__blackMeridianSignalFXLoaded)return;window.__blackMeridianSignalFXLoaded=true;const reduced=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(reduced)return;const terms=["NODE_43B","WITNESS_DETECTED","OMEGA_BLACK","NO_FLAGS","GRAVESIGNAL","MERIDIAN/03","ROOK_NULL","APOCRYPHA","SIGNAL_BURIAL","DEAD_DROP","AUTHORITY_UNKNOWN","VERIFY_NOTHING","EVIDENCE_CHAIN","PRESSURE_FUTURE","MEMORY_CLEAN","RELEASE/BURY","LEARN_FEAR","GLASS_PSALM","NULLSEAL","UNSEALED_ARCHIVE"];function layer(c){const e=document.createElement("div");e.className=c;e.setAttribute("aria-hidden","true");document.body.prepend(e);return e}function rnd(i){const b=terms[Math.floor(Math.random()*terms.length)],h=Math.floor(Math.random()*0xffffff).toString(16).toUpperCase().padStart(6,"0");return [b,`${b} // ${h.slice(0,2)}-${h.slice(2,4)}-${h.slice(4,6)}`,`0x${h} :: ${b}`,`${b} [${String(i%99).padStart(2,"0")}]`][Math.floor(Math.random()*4)]}function setup(){if(document.getElementById("bmSignalCanvas"))return;const canvas=document.createElement("canvas");canvas.id="bmSignalCanvas";canvas.setAttribute("aria-hidden","true");document.body.prepend(canvas);layer("bm-vignette-layer");layer("bm-red-pulse-layer");const rain=layer("bm-data-rain"),float=layer("bm-floating-codes");layer("bm-scanline-layer");layer("bm-noise-layer");for(let i=0;i<42;i++){const s=document.createElement("span");s.textContent=rnd(i);s.style.setProperty("--x",`${Math.random()*100}vw`);s.style.setProperty("--dur",`${8+Math.random()*13}s`);s.style.setProperty("--delay",`${-Math.random()*22}s`);s.style.setProperty("--op",`${.14+Math.random()*.48}`);rain.appendChild(s)}for(let i=0;i<26;i++){const s=document.createElement("span");s.textContent=rnd(i+90);s.style.setProperty("--x",`${3+Math.random()*94}vw`);s.style.setProperty("--y",`${6+Math.random()*88}vh`);s.style.setProperty("--dur",`${5+Math.random()*10}s`);s.style.setProperty("--delay",`${-Math.random()*10}s`);float.appendChild(s)}canvasFX(canvas)}function canvasFX(canvas){const ctx=canvas.getContext("2d",{alpha:true});let w=0,h=0,dpr=1,nodes=[];const ptr={x:0,y:0,active:false};function resize(){dpr=Math.min(window.devicePixelRatio||1,2);w=innerWidth||1280;h=innerHeight||720;canvas.width=Math.floor(w*dpr);canvas.height=Math.floor(h*dpr);canvas.style.width=w+"px";canvas.style.height=h+"px";ctx.setTransform(dpr,0,0,dpr,0,0);const count=Math.max(34,Math.min(92,Math.floor((w*h)/18500)));nodes=Array.from({length:count},(_,i)=>({x:Math.random()*w,y:Math.random()*h,vx:(Math.random()-.5)*.18,vy:(Math.random()-.5)*.18,r:Math.random()<.14?2.2:1.25+Math.random()*1.4,red:Math.random()<.22,phase:Math.random()*Math.PI*2,label:terms[i%terms.length]}))}function move(n){n.x+=n.vx;n.y+=n.vy;n.phase+=.012;if(n.x<-30)n.x=w+30;if(n.x>w+30)n.x=-30;if(n.y<-30)n.y=h+30;if(n.y>h+30)n.y=-30;if(ptr.active){const dx=n.x-ptr.x,dy=n.y-ptr.y,dist=Math.sqrt(dx*dx+dy*dy)||1;if(dist<165){const force=(165-dist)/165;n.x+=(dx/dist)*force*.42;n.y+=(dy/dist)*force*.42}}}function draw(){ctx.clearRect(0,0,w,h);const max=w<720?118:155;nodes.forEach(move);ctx.save();ctx.globalAlpha=.13;ctx.strokeStyle="rgba(139,245,181,.35)";ctx.lineWidth=1;for(let x=-80;x<w+80;x+=160){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x+Math.sin(Date.now()*.00012+x)*20,h);ctx.stroke()}ctx.restore();for(let i=0;i<nodes.length;i++){const a=nodes[i];for(let j=i+1;j<nodes.length;j++){const b=nodes[j],dx=a.x-b.x,dy=a.y-b.y,dist=Math.sqrt(dx*dx+dy*dy);if(dist<max){const alpha=(1-dist/max)*.22,red=a.red||b.red;ctx.strokeStyle=red?`rgba(255,43,53,${alpha})`:`rgba(139,245,181,${alpha})`;ctx.lineWidth=red?.75:.55;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();if(red&&Math.random()<.003){ctx.fillStyle="rgba(255,43,53,.75)";const t=Math.random();ctx.beginPath();ctx.arc(a.x+(b.x-a.x)*t,a.y+(b.y-a.y)*t,2.4,0,Math.PI*2);ctx.fill()}}}}for(const n of nodes){const pulse=.5+Math.sin(n.phase)*.5,color=n.red?"255,43,53":"139,245,181";ctx.fillStyle=`rgba(${color},${.35+pulse*.4})`;ctx.shadowColor=`rgba(${color},.55)`;ctx.shadowBlur=n.red?13:10;ctx.beginPath();ctx.arc(n.x,n.y,n.r+pulse*.9,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0}ctx.save();ctx.font="10px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace";for(let k=0;k<5;k++){const n=nodes[(k*13+Math.floor(Date.now()/2500))%nodes.length];if(!n)continue;ctx.fillStyle=n.red?"rgba(255,43,53,.32)":"rgba(139,245,181,.27)";ctx.fillText(n.label,n.x+8,n.y-8)}ctx.restore();requestAnimationFrame(draw)}addEventListener("resize",resize,{passive:true});addEventListener("pointermove",e=>{ptr.x=e.clientX;ptr.y=e.clientY;ptr.active=true},{passive:true});addEventListener("pointerleave",()=>{ptr.active=false},{passive:true});resize();draw()}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",setup,{once:true});else setup()})();
